...
for iCon = 1:analysisParam.nCon;
    ...
    nuc2nucModes(iCon,iTime) = mode(R(R<filterHigh)); % find means of ratios less than filterHigh
...
figure; clf; hold on;
for iCon = 1:analysisParam.nCon;
plot(analysisParam.plotX(1:size(nuc2nucModes,2)),nuc2nucModes(iCon,:),'Color',colors(iCon,:),'LineWidth',2);
end
...
figure; clf; hold on;
for iCon = 1:analysisParam.nCon;
plot(analysisParam.plotX(1:size(nuc2nucModes,2)),nCells(iCon,:),'Color',colors(iCon,:),'LineWidth',2);
end
